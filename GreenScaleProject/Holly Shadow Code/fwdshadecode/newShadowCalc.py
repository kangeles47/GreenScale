...
class Shadow(BaseElement):
    ...
    #    #print "Opengl_available: ", Opengl_available

    def shadowMain(self, opengl_stat, surfaces_dict, weather_slice):
        #
        #print "Reaching Chris' Shadow Code"
        self.Opengl_status = Opengl_available
        shadowRatios = dict()
        ...
        #if self.Opengl_status == True:
        #    print "self.Opengl_status == True"
        #else:
        #    print "self.Opengl_status == False"
        if self.Opengl_status == False:
            print "self.Opengl_status == False"
        ...
        for surface in surfaces_dict:
            ...
            # if su.obj_type == "ExteriorWall" or su.obj_type == "Roof" or su.obj_type == "Ceiling": (depending on Revit output and future work)
            # In gbxml.py Parapets, Facades, Exterior Walls, and Roofs are all mapped to these two surface types (these are all known types as of test model set)
            if su.obj_type == "ExteriorWall" or su.obj_type == "Roof":
                #print "check passed surfaces: ", su.obj_id, su.obj_type
                name = su.cad
                #if su.obj_type == "Roof" and name.find("Rafter") != -1:
                #    continue
                #    print "Passing surface with Rafter in string, ", name

                #for o in su.openings:
                #    print "found one", o.obj_type
                ...
                #if su.obj_id == "su-33":
                #if counter == 33:
                #    print "current_surface_points: ", su.obj_id
                #    print current_surface_points,

                # Set model rotation, positive is clockwise (assuming not needed)
                #myshadow.set_model_rotation(0.0)

                # Sun azimuth/tilt
                #myshadow.set_sun(np.array([azimuth, tilt]))

                # Put surfaces into main list
                self.add_surface(current_surface_points)

                # Set model rotation after setting surfaces
                # first argument surface, second reported azimuth in degrees, clockwise positive (assuming not needed)
                #myshadow.set_model_rotation_with_azimuth(su, su.azimuth)

                # Add this surface to a dictionary to be looked up later in Temperature.py for each weather_slice
                #shadowRatios[su.obj_id] = myshadow.get_shadow_ratio(0, su) # For openings
                #shadowRatios[su.obj_id] = myshadow.get_surface_shadow_ratio(item) # For surfaces
        ...
        #test geometry ratio
        #print "Shadow for 0 ", myshadow.get_shadow_ratio(0, su) # Use dictionary to hold all ratios
        #print myshadow.normals[2], atan2(myshadow.normals[2][0],myshadow.normals[2][1])

        i = 0
        #print "len", len(self.surfaces)
        #print "len", len(listIndex)
        #print "len", len(surfList)
        for surface in self.surfaces:
            #print "i", i
            shadowRatios[listIndex[i]] = self.get_shadow_ratio(i, surfList[i]) # For openings originally
            #shadowRatios[listIndex[i]] = myshadow.get_surface_shadow_ratio(surface) # For surfaces originally
            #print "stored ratios: ", listIndex[i], shadowRatios[listIndex[i]]
