...
File =  dir('C:\Users\Benoit\SkyDrive\PyCharmProjects\GreenScaleV1\matlab model\Version1-05-09\Weather'); 
n = length(File); 
...
if strcmp(City_name, City); 
   fid = fopen(strcat('C:\Users\Benoit\SkyDrive\PyCharmProjects\GreenScaleV1\matlab model\Version1-05-09\Weather\',File(i, 1).name));
   break; 
end 
...
% Remove the 6th column of the file
C={C{1:5},C{7:35}};
for i=1:34
for j=1:8760
...
%% For every day, depending on the month, add the number of days since January 1st to the day count
for i = 1:m2-m1+1
...
elseif  BB(i,2) == 11
      ...
      BB(i,38)=acos((sin(BB(i,36))-sin(BB(i,37))*sin(Lat))/(cos(BB(i,37))*cos(Lat)));  % azimuth angle   (0, 2pi) from no rth clockwise direction  
