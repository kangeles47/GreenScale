function [Infor_space_simp, Infor_surface_simp, Infor_cons_simp, Infor_win_simp]=SimplifiedGeometryData_fn3(tree,Tem_space)

%Use this to simplify geometry detail
%Modify from SimplifiedGeometryData_fn
%Date: 09/05/2013
...
for i_space=1:Num_space
    ...
    for j_space=1:Num_space
        if (i_space ~= j_space) && ~isempty(intersect(Infor_space_simp(i_space).RelatedSurface,Infor_space_simp(j_space).RelatedSurface))
            ...
            ii=ii+1;
            NeighborSpace(ii)=j_space;  %#ok<AGROW>
        end
    end
    
    Infor_space_simp(i_space).NeighborSpace=NeighborSpace;
...
for i_win=1:Num_win
    ...
    %Row 1 == Value
    Infor_win_simp(i_win).SHGC(1,:)=[tree.WindowType(i_win,1).SolarHeatGainCoeff(1:end-1, 1).CONTENT];
    ...
    if surf_class~=4
        ...
        for i_cons=1:Num_cons
            ...
            if strcmp(Infor_cons_simp(i_cons).id,Infor_surface_simp(i_surface).ConsId)
                ...
                %C
                Infor_surface_simp(i_surface).C=Infor_cons_simp(i_cons).C;
            ...
            if Open_name_id{end-1}=='W'
                ...
                for i_cons=1:Num_cons
                    ...
                    if strcmp(Infor_cons_simp(i_cons).id,Infor_surface_simp(i_surface).Opening(i_open).TypeId)
                        ...
                        %C
                        Infor_surface_simp(i_surface).Opening(i_open).C=Infor_cons_simp(i_cons).C;
